...
class FedAvgClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config, global_logits=None) -> Tuple[Any, int, dict]:
        if config["server_round"] == 1:
            ...
            log(INFO, f"Metric list: {self._metric_list}")
            director = Director()
            # if self._model_name == "mlp":
            self._model = director.create_mlp(self._x_train.shape[1], self._y_train.shape[1], config)
        ...
        results_before_training = evaluator(self._x_test,
                                            self._y_test,
                                            self._model,
                                            self._metric_list)
        log(INFO, f"Evaluated model before: {results_before_training.get_value_of_metric('CrossEntropyLoss')}")

        self._model.fit(self._x_train, self._y_train, epochs=5,
                        batch_size=self._batch_size,
                        callbacks=my_callbacks)

        results_after_training = evaluator(self._x_test,
                                           self._y_test,
                                           self._model,
                                           self._metric_list)
        log(INFO, f"Evaluated model after: {results_after_training.get_value_of_metric('CrossEntropyLoss')}")
    ...
    def evaluate(self, parameters, config):
        ...
        flower_metrics = evaluator(self._x_test, self._y_test, self._model, self._metric_list)

        if config["compute_shapley_values"] == 1:
            client_weights = pickle.loads(config["pickled_client_weights"])
        ...
        return round_result.get_value_of_metric("CrossEntropyLoss"), len(self._x_test), metric_results
