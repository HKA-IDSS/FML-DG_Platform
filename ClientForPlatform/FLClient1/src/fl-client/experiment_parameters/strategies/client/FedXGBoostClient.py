...
class FedXGBoostClient(fl.client.Client):
    ...
    def __init__(self, model, x_train, x_test, y_train, y_test, client_number, metrics, total_data_size=None):
    ...
    def evaluate(self, ins: EvaluateIns):
        ...
        if ins.config["compute_shapley_values"] == 1:
            client_weights = pickle.loads(ins.config["pickled_client_weights"])
