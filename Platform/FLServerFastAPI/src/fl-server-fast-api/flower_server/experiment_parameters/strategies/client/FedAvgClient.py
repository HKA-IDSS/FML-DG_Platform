...
class FedAvgClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config, global_logits=None) -> Tuple[Any, int, dict]:
        ...
        # if config["compute_shapley_values"] == 1:
        #     metrics[str(self._client_number) + "_data_size"] = self._x_train.shape[0]
            gradients = retrieve_gradient_from_dataset(self._model, self._x_train, self._y_train)
    ...
    def evaluate(self, parameters, config):
        ...
        # loss, accuracy, mcc_result = flower_metrics[0], flower_metrics[1], flower_metrics[2]
